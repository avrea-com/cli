...
# Subset of platform_conclusion values that count as "failed" for job-level
# filtering (`avr run logs --failed`). The job table's `platform_conclusion`
# CHECK constraint allows only these seven values; `stale` and
# `startup_failure` are workflow_run-only and would never appear here.
